...
- Encode the computed digest with CESR (text fully qualified Base64) [CESR] to create the final derived and encoded SAID of the same total length as the dummy string and the copied embedded SAID.
...
~~~
The Blake2-256 digest is then computed on the above string and encoded in `CESR` format. This gives the following `SAID`:
...
## Example Python dict to JSON Serializaion with SAID
